  ...
  const cardsData = [
    {
      ...
      stageColor: "bg-amber-50 text-amber-800 border-amber-200/50"
    },
    {
      id: 2,
      ...
      stageColor: "bg-blue-50 text-blue-800 border-blue-200/50"
    },
    {
      id: 3,
      ...
      stageColor: "bg-[#DCF8C6] text-[#128C7E] border-[#25D366]/20"
    }
  ];

  return (
    <section id="solutions-section" className="relative py-24 bg-white border-t border-zinc-100">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="lg:col-span-7">
            <div className="glass-card rounded-3xl p-5 border border-zinc-200 bg-white shadow-xl relative h-full">
              <div className="flex items-center justify-between pb-4 border-b border-zinc-200 mb-6">
                <span className="text-sm font-medium text-zinc-700 uppercase tracking-widest">Active Operations Pipeline</span>
                <span className="text-xs text-zinc-400 font-mono font-medium">Select Row</span>
              </div>
 
              {/* Pipeline Columns Header */}
              <div className="grid grid-cols-3 gap-4 mb-4">
                {['onboarding', 'negotiation', 'completed'].map((col) => (
                  <div key={col} className="text-xs font-medium text-zinc-500 uppercase tracking-wider text-center border-b border-zinc-100 pb-2">
                    {col}
                  </div>
                ))}
              </div>
                    ...
                    <div 
                      ...
                      className={`p-3.5 rounded-xl border text-left cursor-pointer transition-all duration-300 ${
                        selectedCard === card.id 
                          ? 'bg-[#F7FDF9] border-brand-primary/50 ring-1 ring-brand-primary/10 scale-[1.02] shadow-sm' 
                          : 'bg-white border-zinc-200 hover:bg-zinc-50'
                      }`}
                    >
                      <h4 className="text-sm font-black text-[#1A1A2E] mb-1 truncate">{card.name}</h4>
                      <p className="text-sm font-medium text-zinc-500 mb-2">{card.role}</p>
                      <span className={`inline-block text-xs font-medium px-2 py-0.5 rounded-full border ${card.stageColor}`}>
                        {card.status}
                      </span>
                    </div>
                  ))}
                  <div className="h-20 border border-dashed border-zinc-200 rounded-xl flex items-center justify-center text-xs text-zinc-500 font-medium select-none">
                    + Add Record
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  {cardsData.filter(c => c.status === "Pipeline Closed").map(card => (
                    <div 
                      ...
                      className={`p-3.5 rounded-xl border text-left cursor-pointer transition-all duration-300 ${
                        selectedCard === card.id 
                          ? 'bg-[#F7FDF9] border-[#25D366]/40 ring-1 ring-[#25D366]/10 scale-[1.02] shadow-sm' 
                          : 'bg-white border-zinc-200 hover:bg-zinc-50'
                      }`}
                    >
                      <h4 className="text-sm font-black text-[#1A1A2E] mb-1 truncate">{card.name}</h4>
          ...
          <div className="lg:col-span-5">
            <div className="glass-card rounded-3xl p-6 border border-zinc-200 bg-white shadow-md h-full">
              {(() => {
                  ...
                  <div className="space-y-5 animate-fade-in">
                    <div className="pb-4 border-b border-zinc-200 flex items-center justify-between">
                      <div className="space-y-1">
                        <span className="text-xs font-medium text-zinc-400 uppercase tracking-widest">Unified Profile</span>
                        <h3 className="text-lg font-black text-[#1A1A2E]">{currentData.name}</h3>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <span className="text-xs text-zinc-400 uppercase font-medium block mb-0.5">Work Email</span>
                        <span className="text-zinc-600 font-mono break-all font-semibold">{currentData.email}</span>
                      </div>
                      <div>
                        <span className="text-xs text-zinc-400 uppercase font-medium block mb-0.5">Mobile Number</span>
                        <span className="text-zinc-600 font-mono font-semibold">{currentData.phone}</span>
                      </div>
                    </div>
 
                    <div className="space-y-3">
                      <span className="text-xs font-medium text-zinc-400 uppercase tracking-widest block">Activity & Automation History</span>
                      <div className="space-y-3.5 relative pl-4 border-l border-zinc-200">
                        {currentData.history.map((hist, idx) => (
                          <div key={idx} className="relative text-sm">
                            <div className="absolute left-[-21px] top-[4px] w-2.5 h-2.5 rounded-full bg-brand-primary border-2 border-white shrink-0" />
                            <div className="text-zinc-600 leading-snug">{hist.event}</div>
                            <div className="text-sm text-zinc-400 mt-0.5">{hist.time}</div>
                          </div>
                        ))}
                      </div>
                    </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-12">
          {features.map((item, idx) => (
            <div key={idx} className="premium-card p-6 space-y-3">
              <div className="w-10 h-10 rounded-xl bg-white border border-zinc-200 flex items-center justify-center relative shadow-sm overflow-hidden shrink-0">
                <div className="absolute inset-0 bg-brand-gradient opacity-10" />
                <span className="relative z-10">{item.icon}</span>
              </div>
              <h4 className="text-base font-black text-[#1A1A2E]">{item.title}</h4>
