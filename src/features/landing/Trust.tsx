  ...
  return (
    <section className="relative py-24 bg-[#101010] border-t border-zinc-900">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Core Stats Bar */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 pb-16 border-b border-zinc-900 mb-20 text-center">
          {stats.map((stat, idx) => (
            <div key={idx} className="space-y-1">
              <div className="text-3xl sm:text-4xl lg:text-5xl font-black text-gradient tracking-tight">{stat.value}</div>
              <div className="text-sm text-zinc-400 font-medium tracking-wider uppercase">{stat.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <h2 className="text-3xl sm:text-4xl font-bold text-zinc-100 tracking-tight leading-tight">
              If Healthcare Leaders Trust Us With Data, <br />
              <span className="text-gradient">You Can Trust Us With Your Operations.</span>
            </h2>
            <p className="text-zinc-450 text-sm sm:text-base font-semibold leading-relaxed">
            ...
            <div className="flex gap-4 items-center pt-2">
              <div className="flex items-center gap-1.5 bg-zinc-900 border border-zinc-800 px-3.5 py-1.5 rounded-full text-xs text-brand-teal font-medium shadow-sm">
                <ShieldCheck className="w-4 h-4 text-brand-teal shrink-0" /> HIPAA Shielded
              </div>
              <div className="flex items-center gap-1.5 bg-zinc-900 border border-zinc-800 px-3.5 py-1.5 rounded-full text-xs text-brand-primary font-medium shadow-sm">
            ...
            {testimonials.map((test, idx) => (
              <div key={idx} className="premium-card p-6 flex flex-col justify-between border border-zinc-800 bg-[#161616]">
                <div className="space-y-4">
                  ...
                  <p className="text-zinc-350 text-sm font-semibold leading-relaxed italic">
                    "{test.quote}"
                  </p>
                </div>
                <div className="pt-6 border-t border-zinc-800 mt-6">
                  <h4 className="text-sm font-black text-zinc-200">{test.author}</h4>
                  <p className="text-sm font-medium text-zinc-450 mt-0.5">{test.role}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Logo ribbon */}
        <div className="mt-20 pt-10 border-t border-zinc-900 text-center space-y-4">
          <span className="text-xs text-zinc-550 font-semibold uppercase tracking-widest block">INTEGRATED ACROSS LEADING ENTERPRISES</span>
          <div className="flex flex-wrap justify-center items-center gap-x-12 gap-y-6 select-none font-sans font-bold text-sm tracking-widest text-zinc-400 bg-zinc-900/60 py-6 px-8 rounded-2xl border border-zinc-850">
